(ns nonstoreops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (NonStoreRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the NonStoreRetailGovernor (:govern)
  ...
  human door-to-door-route/vending-machine-fleet operations coordinator.
...
(defn build
  ...
  `nonstoreops.store/Store`).
  opts:
    :advisor      -- a `nonstoreops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; NonStoreRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; NonStoreRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
